...
const DisplayCart = () => {
  ...
  return (
    <section className=" min-h-screen w-full">
      <div className="grid grid-cols-3 p-3 border-b-2 border-gray-300">
      ...
      {cart.filter(Boolean).map((item) => (
        <div key={item.id} className="grid grid-cols-3 p-3">
          <span>{item.name}</span>
      ...
      <div className="flex items-center justify-center">
        <button
          ...
          className="cursor-pointer hover:bg-blue-500 bg-blue-600 px-4 py-2 text-white rounded-lg "
        >
          Complete Sale
        </button>
      </div>
